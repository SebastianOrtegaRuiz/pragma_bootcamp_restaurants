...
public class Dishes_OrdersRestController {
    ...
    @PostMapping("/")
    public ResponseEntity<Void> saveDishes_Orders(@ApiParam(value = "require only the units in JSON format Object to save an order", required = true)
                                                  @RequestBody List<Dishes_OrdersRequestDto> dishes_ordersRequestDto) {
